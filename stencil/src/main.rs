use anyhow::Result;
use cardstack_lib::{
    card::{Card, CardEnvelope, Facets, TemplateConstraints, TemplateFacet},
...
enum Commands {
    ...
    /// Interactive TUI for template management
    Tui,
}
...
fn validate_card_against_template(
    card: &Card,
    template: &Card,
            ...
            if let Some(constraints) = &template_facet.constraints {
                ...
                for (field_path, allowed_values) in &constraints.enum_fields {
                    let field_value: Option<serde_json::Value> = if field_path == "title" {
                        ...
                        card.fields
                            .get(field_path.strip_prefix("fields.").unwrap())
                            .cloned()
                    } else {
                ...
                for frozen in &constraints.frozen_fields {
                    if frozen == "author.id" {
                        ...
                        warnings.push(format!(
                            "Field {} should not be modified after creation",
                            frozen
                        ));
                    }
                }
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Commands::New {
            ...
            let constraints = if !required_field.is_empty()
                || !enum_fields_map.is_empty()
                || !frozen_field.is_empty()
            {
        ...
        Commands::List => {
            ...
            let all_cards: Vec<Card> = all_cards_with_paths
                ...
                .map(|(_, card)| card)
                .collect();
            let templates: Vec<_> = all_cards.iter().filter(|c| c.has_template()).collect();
            ...
            if cli.jsonl {
                ...
                for template in templates {
                    println!(
                        "  {} - {} ({})",
                        template.slug, template.title, template.uid
                    );
                }
            }
        }
                    ...
                    if let Some(template_facet) = &facets.template {
                        ...
                        if let Some(constraints) = &template_facet.constraints {
                            ...
                            if !constraints.frozen_fields.is_empty() {
                                println!(
                                    "  Frozen fields: {}",
                                    constraints.frozen_fields.join(", ")
                                );
                            }
                        }
                    }
                    ...
                    if let Ok(template) = load_card(&repo, template_uid) {
                        if template.has_template() {
                            let (valid, errors, warnings) =
                                validate_card_against_template(&card, &template);
        ...
        Commands::Tui => {
            ...
            let all_cards: Vec<Card> = all_cards_with_paths
                ...
                .map(|(_, card)| card)
                .collect();
            let templates: Vec<Card> = all_cards
                .iter()
                .filter(|c| c.has_template())
                .cloned()
                .collect();

            tui::run_tui(templates, all_cards)?;
        }
    }
