...
fn import_jsonl(repo: &Path, input_dir: &Path, template: Option<&str>, _anonymize: bool) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        // Convert envelope back to card
        let mut card = Card::new(
            envelope.title.clone(),
            envelope.slug.clone(),
            envelope.uid.clone(),
        );
        card.created = envelope.created;
        card.updated = envelope.updated;
        card.tags = envelope.tags;
        card.fields = envelope.fields;
        
        // Set content if available
        if let Some(facets_json) = &envelope.facets {
            if let Ok(facets) = serde_json::from_value::<cardstack_lib::card::Facets>(facets_json.clone()) {
                if let Some(content) = &facets.content {
                    card = card.with_content(content.body.clone());
                }
            }
        }
        ...
        if let Some(template_slug) = template {
            // TODO: Load template and apply defaults/constraints
            // For now, just note it
            card.fields.insert("_template".to_string(), serde_json::Value::String(template_slug.to_string()));
        }
...
fn import_csv(repo: &Path, input_dir: &Path, template: Option<&str>, _anonymize: bool) -> Result<()> {
...
fn import_markdown(repo: &Path, input_dir: &Path, template: Option<&str>, _anonymize: bool) -> Result<()> {
    let mut count = 0;
    ...
    for entry in WalkDir::new(input_dir) {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("md") {
            ...
            if content.starts_with("---\n") {
                if let Some((front_matter, body)) = content.splitn(3, "---\n").nth(2).and_then(|rest| {
                    ...
                    let uid = front.get("uid")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| uid::generate_uid());
                    let slug = front.get("slug")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| {
                            path.file_stem()
                    ...
                    if let Some(fields) = front.get("fields").and_then(|v| v.as_mapping()) {
                        for (k, v) in fields {
                            if let Some(key) = k.as_str() {
                                card.fields.insert(key.to_string(), serde_json::to_value(v)?);
                            }
                        }
                    }
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        ...
        Commands::Import { from, r#in, template } => {
            match from.as_str() {
